...
export default function CertificationTasksPage({ user, notify, realtimeVersion, openCertificateForm, personalMode = false }) {
  ...
  const userStatusToggle = <div className="completion-chart-toggle" role="group" aria-label="Filter courses by status">
    ...
    <button type="button" className={statusFilter === 'under_review' ? 'active' : ''} onClick={() => setStatusFilter('under_review')}>Under Review</button>
    <button type="button" className={statusFilter === 'validated' ? 'active' : ''} onClick={() => setStatusFilter('validated')}>Validated</button>
  </div>
  ...
  return <section className={`certification-tasks-page ${isAdmin ? 'admin-courses-page' : 'user-courses-page'}`}>
    ...
    {isAdmin ? <section className="task-course-filter-panel">
