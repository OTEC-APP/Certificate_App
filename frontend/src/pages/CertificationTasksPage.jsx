...
export default function CertificationTasksPage({ user, notify, realtimeVersion, openCertificateForm }) {
  ...
  const pending = useMemo(() => filteredTasks.filter((task) => !task.completed).length, [filteredTasks])
  ...
  const pagedPendingTasks = useMemo(() => pendingTasks.slice((pendingPage - 1) * coursePageSize, pendingPage * coursePageSize), [coursePageSize, pendingPage, pendingTasks])
  const pagedCompletedTasks = useMemo(() => completedTasks.slice((completedPage - 1) * coursePageSize, completedPage * coursePageSize), [completedPage, completedTasks, coursePageSize])

  useEffect(() => { setCoursePage(1); setPendingPage(1); setCompletedPage(1) }, [search, oemFilter, categoryFilter, statusFilter, isAdmin])
  ...
  const taskCard = (task) => <article className={`task-card ${task.completed ? 'completed' : ''}`} key={task.id}>
    ...
    <footer>{!isAdmin && <a href={task.certification_link} target="_blank" rel="noreferrer"><i className="bi bi-box-arrow-up-right" /> Open certification</a>}{!isAdmin && (task.completed ? <span className="task-approved"><i className="bi bi-patch-check-fill" /> Admin approved</span> : task.submitted ? <button type="button" className="task-under-review" disabled><i className="bi bi-hourglass-split" /> Under review</button> : <button type="button" onClick={() => openCertificateForm(task)}><i className="bi bi-upload" /> Add certificate</button>)}</footer>
  </article>

  const userTaskTable = (items) => <div className="course-table-wrap"><table className="course-table"><thead><tr><th>Course</th><th>OEM</th><th>Category</th><th>Due date</th><th>Status</th><th>Action</th></tr></thead><tbody>{items.map((task) => <tr key={task.id}><td data-label="Course"><b>{task.certification_name}</b></td><td data-label="OEM">{task.vendor_name || 'Not specified'}</td><td data-label="Category">{task.category || 'Other'}</td><td data-label="Due date">{formatDate(task.due_date)}</td><td data-label="Status"><span className={task.completed ? 'done' : 'pending'}>{task.completed ? 'Completed' : task.submitted ? 'Under review' : 'Pending'}</span></td><td data-label="Action">{task.completed ? <span className="course-action-muted">Approved</span> : task.submitted ? <span className="course-action-muted">Under review</span> : <div className="course-user-actions"><a href={task.certification_link} target="_blank" rel="noreferrer" style={{ gap: 6, minWidth: 172 }}><i className="bi bi-box-arrow-up-right" /><span>Open certificate</span></a><button type="button" className="course-open-action" onClick={() => openCertificateForm(task)} style={{ gap: 6, minWidth: 162 }}><i className="bi bi-cloud-arrow-up" /><span>Add certificate</span></button></div>}</td></tr>)}</tbody></table></div>
  ...
  return <section className={`certification-tasks-page ${isAdmin ? 'admin-courses-page' : 'user-courses-page'}`}>
    ...
    <section className="task-course-filter-panel">
      ...
      {showCourseFilters && <div className="task-course-filters" role="search" aria-label="Filter courses">
        ...
        <label><span>Status</span><CompactSelect value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All courses</option><option value="pending">Pending</option><option value="completed">Completed</option></CompactSelect></label>
    ...
    {loading ? <p className="task-empty">Loading certification tasks...</p> : isAdmin ? <section className="task-list-section"><header><div><span>COURSE LIBRARY</span><h2>Courses</h2></div><b>{filteredTasks.length} courses</b></header>{filteredTasks.length ? <><div className="task-card-grid">{pagedTasks.map(taskCard)}</div><Pagination page={coursePage} totalItems={filteredTasks.length} pageSize={coursePageSize} onPageChange={setCoursePage} onPageSizeChange={setCoursePageSize} label="courses" /></> : <p className="task-empty">No courses match the selected filters.</p>}</section> : <><section className="task-list-section course-table-section"><header><div><span>NEEDS ATTENTION</span><h2>Pending tasks</h2></div><b>{pendingTasks.length} pending</b></header>{pendingTasks.length ? <>{userTaskTable(pagedPendingTasks)}<Pagination page={pendingPage} totalItems={pendingTasks.length} pageSize={coursePageSize} onPageChange={setPendingPage} onPageSizeChange={setCoursePageSize} label="pending courses" /></> : <p className="task-empty success"><i className="bi bi-check-circle" /> You have completed every assigned task.</p>}</section>{completedTasks.length > 0 && <section className="task-list-section course-table-section completed-list"><header><div><span>COMPLETED</span><h2>Completed tasks</h2></div><b>{completedTasks.length} completed</b></header>{userTaskTable(pagedCompletedTasks)}<Pagination page={completedPage} totalItems={completedTasks.length} pageSize={coursePageSize} onPageChange={setCompletedPage} onPageSizeChange={setCoursePageSize} label="completed courses" /></section>}</>}
  </section>
}
